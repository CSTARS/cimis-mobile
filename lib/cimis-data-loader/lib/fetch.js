...
function aggregate(layer) {
  for( var key in layer.aggregate ) {
    var len = layer.aggregate[key].length;
    var total = 0;
    layer.aggregate[key].forEach((val) => {
      total += val; 
    });
    layer.aggregate[key] = total / len;
  }
}
...
function prepareAggregate(layer, row, col, pixel) {
  ...
  for( var region in regions ) {
    ...
    layer.aggregate[region].push(pixel * (parseInt(regions[region])/100) );
  }
}
